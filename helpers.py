...
def calc_sil(r=None, x=None, b=None, tap=None, shift=None, **kwargs):
    ...
    """
    zabs = np.sqrt(r**2 + x**2)

    # filter out transformer branches (off nominal tap, shift, and zero b)
    idx  = np.where((tap== 1) & (shift == 0) & (b != 0) )[0]
    return dict(zip(idx, np.sqrt(np.abs(b[idx])/zabs[idx])))
